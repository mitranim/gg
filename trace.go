...
var (
	...
	TraceBufSize   = 64 // Default buffer capacity in `CaptureTrace`.
)
...
/*
Shortcut for capturing a trace of the the current call stack, skipping the
specified number of frames, where 0 corresponds to the caller's frame.
The frame of `CaptureTrace` is not captured.
*/
func CaptureTrace(skip int) Trace {
	chunkSize := TraceBufSize
	var buf Trace
	var off int

	for {
		buf = GrowLen(buf, chunkSize)
		size := runtime.Callers(skip+off+2, buf[off:])
		if size <= 0 || Last(buf) == 0 {
			return buf[:off+size]
		}
		off += chunkSize
	}
...
/*
Represents a Go stack trace. Typedef of `[]uintptr` with various methods for
capturing and printing stack traces.

Also see the type `Caller`, a typedef of `uintptr`, used internally by `Trace`.
*/
type Trace []uintptr

/*
Uses `runtime.Callers` to capture the current call stack into the given `Trace`
up to its length. The returned slice is truncated. To capture a full trace
without length limitations, use `CaptureTrace`.
*/
func (self Trace) Capture(skip int) Trace {
	return self[:runtime.Callers(skip+2, self)]
}
...
func (self Trace) AppendIndentMultiTo(buf []byte, lvl int) []byte {
	for _, val := range self {
		buf = Caller(val).AppendNewlineIndentTo(buf, lvl)
	}
	return buf
}
...
func (self Trace) IsNotEmpty() bool { return Some(self, IsNotZero[uintptr]) }

// Converts to `Frames`, which is used for formatting.
func (self Trace) Frames() Frames { return Map(self, callerFrame) }

func callerFrame(val uintptr) Frame { return Caller(val).Frame() }
...
func (self Caller) Func() *runtime.Func {
	if IsZero(self) {
		return nil
	}
	return runtime.FuncForPC(self.Pc())
}
